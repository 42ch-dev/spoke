...
**Ship the convenience layer as a header-only wrapper, not a second library.** Once the C ABI exists, the host-language ergonomics belong in one hand-written header that includes it: move-only owning types for the C resources (the output buffer, the opaque handles), the language's optional type for the C optional-value convention, a `Result`-style return for every status, borrowed length-authoritative text views with an explicit owned copy, and callback records built from function objects and transferred through an owning pointer only after the C constructor succeeded. Two rules stay absolute: **destruction frees but never closes a live session** (a router or another handle may still reference it), and neither side of the wrapper may change an exported symbol, a record layout or the ABI revision. A header-only layer adds no artifact and no version-bearing manifest, so it stays inside the existing tag-based distribution and needs no new publish channel.

**Verify the wrapper under both exception configurations, and make the runner's assertions exact.** The interesting failure here is silent. With exceptions disabled, nothing may `throw` across the boundary and the trampoline must preprocess to plain status returns; with them enabled, the trampoline must contain a throwing host callback and map it to the documented status without reclassifying a normal return. Run the same smoke under both flag sets — they are two real code paths, not a platform matrix. Then require each configuration's printed assertion banners to equal its expected list **exactly**: a lenient "every expected banner appeared, in order" check happily accepts a banner leaked from the other configuration, which is precisely the build-guard failure the two-configuration run exists to catch.

**Keep the foreign-boundary's awkward cases distinct.** A zero-length foreign buffer is not an empty buffer — if the carrier does not release a zero-length record, allocating a holder for it leaks, so emit the zero record instead. Likewise, a present-but-empty optional field and an absent one are different values on this boundary and must survive the wrapper unmapped.
