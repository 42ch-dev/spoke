...
3. **Tag-triggered release** — `.github/workflows/release.yml` on `push.tags: ['v*']`. Four parallel verify jobs (including `verify-version`) → sequential `release` job with `needs:` (fail-closed). Creates GitHub Release from annotated tag; on stable tags, publishes `@42ch/spoke-schemas` and `@42ch/spoke-operations` to npm via Trusted Publisher OIDC (`publish-npm`: `id-token: write`, Node 24, pack with pnpm + `npm publish` tarball), then `spoke-schemas` and `spoke-operations` to crates.io (`publish-crates` runs after `publish-npm`; schemas before ops in each registry).
