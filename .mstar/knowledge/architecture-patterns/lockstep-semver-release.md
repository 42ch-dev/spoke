...
3. **Tag-triggered release** — `.github/workflows/release.yml` is top-level only (`push.tags: ['v*']`, `pull_request` closed + label `release`, or `workflow_dispatch` with `tag`). No `workflow_call` wrapper — Trusted Publishing OIDC binds to filename `release.yml` (mstar-harness pattern). Verify jobs → GitHub Release → npm (Node 24 + `registry-url`, stock npm) → crates.io (`crates-io-auth-action`); schemas before ops in each registry.
