...
The repository maintains **23** hand-authored `schemas/**/*.schema.json` files. `assert-schema-count.mjs` and `rust-gen` both assert exactly **23** generated output files — bump both constants in the same commit when the inventory changes.
