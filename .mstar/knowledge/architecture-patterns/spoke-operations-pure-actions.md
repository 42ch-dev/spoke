...
- Uniqueness helpers that take `block_type`/`canonical_name` **params** must cross-check against `candidate` wire fields (`INVALID_INPUT`) — otherwise callers can bypass the gate.
