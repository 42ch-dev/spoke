...
Field name on wire: **`timeline_scale`** (common `TimelineScale` def; optional on `TimelineEvent` and `Scope`).
...
- Schema: `schemas/common/common.schema.json` (`TimelineScale`), `schemas/data/timeline-event.schema.json`
