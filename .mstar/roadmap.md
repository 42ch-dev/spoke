...
|------|-------|-------------|
| 2026-08-03 | Swift `SpokeConnect` SPM + Kotlin Maven | Root `Package.swift` product `SpokeConnect` + committed xcframework; `io.github.42ch-dev:spoke-connect` on GitHub Packages with JNA natives + `publish-maven` on `release.yml`; golden-parity smokes for both |
