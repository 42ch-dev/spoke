...
| **Connect envelope-authentication hardening** | Authenticate every connect envelope beyond the signed hello: sign/MAC the `ConnectSession` snapshot + the `ConnectInvokeRequest`/`Response` so the protocol is secure-by-default over unauthenticated transports (currently the model delegates channel binding to the transport — TLS/Noise). A breaking multi-step protocol change (extend signed-field sets or add a session-derived MAC key via a handshake key exchange). Trigger: a product host requires protocol-level channel authentication independent of the transport layer | Greptile P1 "Session snapshot remains unauthenticated" — the dial-binding (responder signs initiator nonce) closed the hello-replay surface in v0-iter030; the session + invoke envelopes remain transport-authenticated only |
