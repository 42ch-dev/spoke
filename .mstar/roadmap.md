...
| **spoke-connect uniffi bindings (multi-language)** | Swift sync-core skeleton landed; next binding targets by product priority: **C#, Go, Python, Swift, Kotlin** (C# first, deferred pending bindgen-cs uniffi 0.32 support) | Priority set by product direction (2026-08-02); **C# deferred** — `uniffi-bindgen-cs` v0.11.0 targets uniffi 0.31 vs the repo's pinned uniffi 0.32 (see [`connect-csharp-bindgen-deferred.md`](specs/connect-csharp-bindgen-deferred.md)); revisit trigger: a bindgen-cs tag (or main commit) targeting uniffi 0.32+; Go/Python community bindgens carry the same version-gap risk until they catch up — verify against uniffi 0.32 before binding work; async-FFI options documented in crate README; DHT/NAT discovery remains future |
