...
|-------|------------|-------|
| **connect publish Stage 1** | First registry publish of `@42ch/spoke-connect-ts` (npm) per [connect-publish-strategy.md](specs/connect-publish-strategy.md); dist/ + packed-tarball smokes; extend `release.yml` publish-npm list + Trusted Publishing; co-update `spoke-version-release.md` + `AGENTS.md` connect boundary | **Trigger:** published-shape checklist complete + golden/suite green + maintainer release cut. Keep `spoke-connect` crate and language bindings unpublished unless Stage 2 revises |
| **spoke-connect TS SDK deepen** | Continue pure-TS-minimal path; golden-vector parity with Rust session core; js-libp2p on demand only | Direction locked 2026-08-02: WebSocket default; WebTransport deferred; WASM of Rust core only if pure-TS crypto/JCS diverges |
| **spoke-connect uniffi bindings** | Binding matrix by product priority: **C#, Go, Python, Swift, Kotlin**. Swift sync-core skeleton landed; **C# landed** — generated binding + net8.0 golden smoke (peer_id + hello signature + verify) via the vendored `uniffi-bindgen-cs` fork ([connect-csharp-bindgen-deferred.md](specs/connect-csharp-bindgen-deferred.md)) | C# generation uses the vendored fork (129-line patch, generation-only) until upstream tags a uniffi 0.32+ release, then drop the patch; Go/Python/Kotlin remain with the same feasibility gate |
| **Integrator docs i18n (EN + CN)** | VitePress `docs/` locale support: English + 中文 for the integrator site (nav, pages, locale switch); keep Pages deploy covering both | Site scaffold is EN-only today; SSOT remains `.mstar/specs/` (English normative). Consumer-facing docs twin the outline in CN |
| **Integrator docs content** | Keep VitePress `docs/` + GitHub Pages aligned with `.mstar/specs/` SSOT (site scaffold already on main) | Content/SSOT sync as profiles and connect docs evolve; pairs with docs i18n above |
