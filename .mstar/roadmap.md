...
|-------|------------|-----------------|
| **Connect integration demo (mock inference host + RemoteAdapter client)** | Runnable TypeScript demo under `examples/connect-demo/`: a mock inference host that serves a real `BaselinePorts` adapter over the connect wire family via WebSocket, and a third-party-style client that dials with `RemoteAdapter` using only `@42ch/spoke-connect` + `@42ch/spoke-schemas` (+ `ws`) and observes mock-inference artifacts | Third-party integrators need an end-to-end runnable reference after RemoteAdapter shipped; the consumer WebSocket `Transport` stays demo-local (productized transports remain under **Per-language WebSocket transports**) |
| **RemoteAdapter integration tutorial (EN+CN)** | Diátaxis tutorial under `docs/tutorials/` (EN + CN twins) that walks a third-party TypeScript developer through implementing a message-oriented `Transport`, dialing `connectRemoteAdapter`, calling `BaselinePorts`, and handling errors — using the shipped connect demo as the runnable spine; Tutorials sidebar + cross-links from `first-connect-session` and the RemoteAdapter how-to | Tutorials currently stop at session primitives; the RemoteAdapter how-to is task-oriented — integrators still need a learning-path tutorial |
