...
|-------|------------|-------|
| **spoke-connect uniffi bindings** | Binding matrix (**C#, Go, Python, Swift, Kotlin**) on four publish channels: **GitHub Packages** (C# NuGet landed, Kotlin Maven in progress), **SPM git** + **Go modules git** (Swift skeleton landed; Go in progress), **PyPI** OIDC (Python in progress). Packaging contract: [connect-binding-channels.md](specs/connect-binding-channels.md), [connect-publish-strategy.md](specs/connect-publish-strategy.md) | Community bindgen tools verified against the uniffi 0.32 pin before each slice; vendored-fork path available ([connect-uniffi-bindgen-fork.md](knowledge/architecture-patterns/connect-uniffi-bindgen-fork.md)) if a tool lags |
...
| Connect wire (opt-in) | `schemas/connect/` six envelopes; hello JCS + auth paths; published `@42ch/spoke-connect` + crates.io `spoke-connect`; Path B bindings on four channels (C# NuGet landed; Kotlin Maven, SPM, Go modules, PyPI per [connect-binding-channels.md](specs/connect-binding-channels.md)) |
