...
|-------|------------|-----------------|
| **`protocol_version_mismatch` alignment** | Either emit a dedicated hello mismatch kind from session-core / RemoteAdapter / FFI, or amend the connect error-mapping vocabulary to the shipped `HandshakeFailed` / `Dial{kind:"handshake"}` behavior | Spec currently lists `protocol_version_mismatch`; shipped code fails mixed-version hellos as handshake. Docs already describe shipped behavior + reserved vocabulary |
| **publish-pypi / publish-maven idempotent skip** | Release re-runs skip Python/PyPI and Kotlin/Maven artifacts already present at the tag SemVer, matching npm + crates.io publish scripts | Remaining Release re-run risk after npm/crates idempotent skip landed with the 0.9.0 lockstep repair |
