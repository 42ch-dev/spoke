...
|-------|------------|-------|
| **connect publish execution** | Stage 1: first registry publish of `@42ch/spoke-connect-ts` (npm) + docs site live per [`.mstar/specs/connect-publish-strategy.md`](specs/connect-publish-strategy.md); extend `release.yml` publish-npm list + Trusted Publishing; keep `spoke-connect` crate and bindings unpublished unless Stage 2 decision revises | **Trigger:** decision record accepted on main + published-shape checklist complete + golden/suite green on main + maintainer release cut. **Owner:** release maintainer / next delivery slice |
