...
|------|-------|-------------|
| 2026-08-02 | spoke-connect multi-language deepening & delivery prep | **C# binding deferred** (bindgen-cs 0.31 vs uniffi 0.32 version gap — decision record + revisit trigger); **publish strategy locked** (informative record: Stage 1 = npm `@42ch/spoke-connect-ts` + Pages docs, dist/ + tarball smokes + release-SSOT co-update mandated; lockstep versioning; bindings never registry-published; WT defer / js-libp2p fallback-only evaluations) + **TS published-shape prep** (exports contract, files/metadata, exports-map smoke); **integrator docs site** (VitePress `docs/`, 17 SSOT-linked pages, GitHub Pages deploy workflow with ref-scoped concurrency); knowledge `connect-publish-staging` + `integrator-docs-site-ssot-links` + FFI-boundary pipeline-verification update |
