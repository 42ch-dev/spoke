...
| **L5 Temporal** | Timeline dimension + Event + optional Fork | **`Event` wire object** (when-axis) + **Timeline projection tiers** (`brief` / `narrative` / `moment`) as structured Timeline vocabulary | **Fork** (world-history branch) — not required for baseline | `event.schema.json` (v0-iter003); Timeline tier vocabulary in this spec + data-model; Fork deferred optional |
| **L6 Constraint** | Rule / Prohibition | **`Rule` wire object** | Prohibition variants as open vocabulary | `rule.schema.json` (v0-iter003); `check` op |
...
**Wire field (architect-locked):** optional `timeline_scale` on `Event` and as an optional `Scope` refinement filter — values `brief` | `narrative` | `moment` (open string; core vocabulary documented, not `enum`). Shared def: `common.schema.json#/definitions/TimelineScale`.
...
| `TimelineScale` | `Event.timeline_scale`, `Scope.timeline_scale` | L5 tier vocabulary (`brief` / `narrative` / `moment`) |
...
- [x] Every baseline layer row maps to at least one committed schema or op family (or explicit “product binding” note for L5 Event consumers)
...
- [x] Layer ↔ artifact matrix is complete (schema / op / library helper per layer)
