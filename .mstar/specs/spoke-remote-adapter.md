...
| `ports: Option<Arc<dyn RemoteServePorts>>` | (no parameter; pinned `None`) | no foreign-callback ports face; `port.*` invokes hit the library's documented absent-ports deny branch (non-goals below); the FFI ports face — a foreign-callback ports parameter wrapping the widened seam — ships in the follow-on plan, with its acceptance rows checked there |
