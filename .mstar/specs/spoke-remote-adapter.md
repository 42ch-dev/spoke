...
- [ ] D15: `invoke_tool` invoked via FFI — `RemoteAdapterFFI.invoke_tool` / `MultiPeerRouterFFI.invoke_tool` generated in all five bindings as synchronous block-on-async calls over the cdylib-owned runtime (JSON-string `arguments_json` in, tool `result` payload out), grammar fail-fast `INVALID_INPUT` with zero wire traffic, router terminal `no_capable_peer` reject, D7-by-reference error rows
- [ ] D16: `tools.*` served via FFI — `ToolHandler` callback interface, `RemoteAdapterFFI.register_tool_handler` (dialer-side serving), and `ConnectResponderFFI` (`connect_responder_ffi` factory, session info, `register_tool_handler` / `invoke_tool` / `close`) generated in all five bindings; accept topology = host-owned listen/accept wrapping a connected callback `Transport` (loopback proofs over `loopback_transport_pair()`)
- [ ] Error rows: the D15/D16 `FfiError` rows hold at the bindings — grammar rejects, dispatch deny (`wire_code: "op_unsupported"`), envelope-auth kinds, `invalid_sequence` wire_code, handler `Rejected` pass-through with `Dial` / exception / panic containment, config-validation `Dial { kind: "config" }`, responder handshake failure via `state → "Closed"` (no error row)
- [ ] Containment: panics inside block-on-async futures and non-contract handler failures never unwind across the FFI boundary (`INTERNAL_ERROR` containment rows; the serve loop survives); concurrent calls from multiple foreign threads multiplex over the shared runtime
