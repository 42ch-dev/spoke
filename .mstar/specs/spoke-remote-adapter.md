...
The FFI surface (D12/D15/D16) carries the session faces — `RemoteAdapterFFI` / `MultiPeerRouterFFI` / `ConnectResponderFFI`, the callback `Transport` and `ToolHandler` interfaces, `invoke_tool`, and `register_tool_handler`. The pure validators `validateManifestTools` and `validateToolArguments` are library-level host-side helpers in `@42ch/spoke-operations` / `spoke-operations` (public API in both languages, with dedicated test coverage) and are not exported over FFI. They sit outside the parity surface as thin conveniences (the root `AGENTS.md` session-core parity paragraph: thin client conveniences sit outside the parity surface), and the connect library adds no pure helpers of its own (`spoke-connect.md` §Relationship to the existing three pillars: connect does not add pure helpers; remote dispatch stays adapter-owned). Native hosts' manifest correctness is enforced at handshake and dispatch time instead: the signed hello authenticates the embedded manifest at session establish, and the `tools.*` dispatch gate denies a manifest-declared-but-unregistered tool fail-closed (`op_unsupported` → `CAPABILITY_PORT_MISSING`, D13 deny matrix) at invoke time. Hosts that want pre-dial or pre-invoke validation call the operations-library helpers directly in their own language.
