...
| Foreign `ToolHandler` returns `Err(Rejected{..})` | Application reject passes through verbatim — `code` / `message` preserved; `kind` / `wire_code` preserved on the `FfiError::Rejected` top-level fields (the inverse of `map_spoke_reject`) |
