...
Two remote shapes share one open vocabulary and one envelope family:

- **Port-method ops** — one `port.*` invoke per `BaselinePorts` method, so the caller keeps the orchestration (`orchestrate*` runs locally) and performs remote I/O per port call.
- **Whole-operation ops** — one core-op invoke whose `payload` is an existing ops request envelope and whose success `payload` is that operation’s response envelope, so the **serving** host runs the orchestration. `extract` is the shipped row of this shape.
...
Whole-operation ops use the core `op` string itself — no `port.*` prefix and no entry in the port-method catalogue — which is why the core ops vocabulary stays available to the remote surface. Required capability for baseline port ops: `spoke-baseline`, authorized through the existing product `op_capability_requirements` map on hosts; the optional rows register `port.computable.project` / `port.computable.compute` → `l2-computable` and `port.fork.list_timeline_events` → `l5-fork` in the same map.

Catalogue (baseline + optional, shipped):
...
| `project` | `port.computable.project` | `ProjectRequest` (snake_case) | `ProjectResponse` |
| `compute` | `port.computable.compute` | `ComputeRequest` (snake_case) | `ComputeResponse` |
| `listForkTimelineEvents` | `port.fork.list_timeline_events` | `{ "scope": Scope }` — `fork_id` on the `Scope` | `TimelineEvent[]` |
...
Whole-operation rows (shipped):

| Method | `op` | Request `payload` | Success `payload` | Required capability |
|--------|------|-------------------|-------------------|---------------------|
| `extract` | `extract` — a core op, **not** `port.*`, not a `PORT_OPS` catalogue member | `ExtractRequest` itself (`run_id`, `sources`, optional `entry_types` / `extensions`) — no `{ request }` wrapper | `ExtractResponse` success branch `{ candidates, run, extensions? }` — no `{ result }` wrapper | `ke-extraction` |

The method is inherent and async on the adapter in both languages — `RemoteAdapter.extract(request: ExtractRequest): Promise<SpokeResult<ExtractResponse>>` / `RemoteAdapter::extract(&self, request: ExtractRequest) -> SpokeResult<ExtractResponse>` — and deliberately sits outside the `port.*` method catalogue, because it delegates a whole operation rather than one port call. The serving host owns the extraction port, the loaded input value and the run extractor; the request carries no loaded value, no host closure and no callback, so nothing from the in-process loader crosses the wire. A malformed success payload rejects `INTERNAL_ERROR` with `details.kind = "transport"` (the Rust decode failure and the TS success-shape gate agree), and the wire refusal is the D7 dispatch-deny row — the dialer holds no local capability table and never pre-denies. The FFI face of the same operation is D16: the dialer-side `RemoteAdapterFFI.extract(extract_request_json)` and the serving-side `PortsHandler.extract(extract_request_json)`, both synchronous `Result<String, FfiError>` over JSON-string boundaries.

**Scope-bearing rows.** `listKnowledgeEntries` / `listTimelineEvents` / `listForkTimelineEvents` send `{ "scope": <Scope> }` and pass the declared Scope through unchanged (semantic field/value preservation — `viewpoint`, owner and disclosure values are never stripped to make a request succeed). When the declared scope carries a non-empty string `viewpoint`, the request additionally requires the `ke-ownership` capability next to its row capability (`spoke-baseline`, or `l5-fork` for the fork timeline op); the normative predicate lives in [`spoke-connect.md`](spoke-connect.md) §Op dispatch gate, and its placement across dialer / router / responder is in D7 / D11 / D14. `extract` carries no Scope and is never ownership-gated.

Optional families (shipped): `project` / `compute` → `port.computable.*` (required capability `l2-computable`); `listForkTimelineEvents` → `port.fork.list_timeline_events` (required capability `l5-fork`). The optional rows are capability-gated exactly like the baseline rows: the responder's dispatch gate evaluates the required capability against `negotiated_capabilities` (the both-hellos intersection, computed at establish), and a gate deny answers the existing dispatch-deny branch (`op_unsupported` / `capability_missing`), mapped by the dialer through the D7 row to `CAPABILITY_PORT_MISSING` with `details.wire_code` preserved. Serving additionally probes the injected provider for the declared family after the gate passes (D14); a declared-but-not-provided fa…

**No schema shape changes** — open `op` + opaque `payload` already carry this design. The connect invoke-request schema’s `op` `description` lists `extract` among the core values; that is documentation inside the existing annotation and changes no shape, schema count or wire code.
...
| Invoke timeout (local outbound waiter) | `SpokeResult` reject `INTERNAL_ERROR`, `details.kind = "timeout"` — synthesized locally when the dialer's own waiter deadline expires (waiter only; the served peer is not informed) |
| Serve wait expiry (serving side) | **Signed** response from the peer: one error envelope with code `INTERNAL_ERROR` and `details.kind = "timeout"`, which the dialer surfaces through the application-reject row above. The responder's local serve budget bounds provider work (D14 serving) — the session stays `Established`, the consumed inbound sequence is preserved, and no retry / counter rollback / extra response / capability mutation / forced close follows |
...
**Refusal vocabulary (no new wire codes).** Every capability refusal answers the existing wire `op_unsupported` and maps through the dispatch-deny row above to `CAPABILITY_PORT_MISSING` with `details.wire_code` preserved. That single row covers the static core-table requirement (`extract → ke-extraction`), the product port-map families, and the conditional `ke-ownership` requirement a viewpoint-bearing Scope request adds; no new `kind` and no new wire code is introduced, and existing recognition of `capability_missing` from interoperating peers stays intact. A malformed Scope on a Scope-bearing op is the application reject `INVALID_INPUT` — validation before any provider call, not a capability verdict. Router exhaustion is the local terminal `CAPABILITY_PORT_MISSING` with `details.kind = details.wire_code = "no_capable_peer"` (D11): a router label, never a peer-answer code, and never conflated with an injected service's own application reject. The dialer holds no local capability table and never pre-denies an op — a missing requirement always surfaces as the peer's answer through these rows, so no retry against a different peer follows a selected adapter's reject.
...
- **Hard filters:** the peer's `capabilities[]` includes the operation's required capability (`spoke-baseline` for the `upsert` / `promote` / `relate` / `check` / `assemble` families and the `port.*` baseline ops; `l2-computable` for `project` / `compute` and the `port.computable.*` ops; `l5-fork` for `port.fork.list_timeline_events`; `ke-extraction` for the `extract` core op). The `extract` selection row is **inert for the fixed public router surface**: the router keeps its baseline + tools methods and exposes no `extract` method, so the row is exercised through the pure `selectPeerForOp` / `select_peer_for_op` and consumers call `extract` on their per-peer adapter; it carries no preferred role either — serving extraction is a role (`input-source`) that is descriptive, not a selection input, so there is no soft-preference row. When the request declares a non-empty `payload.scope.viewpoint` on a Scope-bearing op, the peer's `capabilities[]` MUST ALSO include `ke-ownership` — the same hard capability filter, never a soft preference, and advertisement alone is not authorization (the selected peer's responder still evaluates the conjunct against its own negotiated set). When the request carries a namespace (derived from the payload `Scope`), the peer's `namespaces[]` includes it exactly — v1 namespace matching is exact, so a peer declaring `namespaces: ["*"]` declares the literal string `"*"`; when the peer manifest and the request both declare `authority.scope_key`, the two match exactly, and a single-sided declaration passes that gate.
...
- **D15/D16 tool faces:** the tool-invoke face `invoke_tool` (`RemoteAdapterFFI` / `MultiPeerRouterFFI`, D15; `ConnectResponderFFI`, D16) and the tool-serving faces `register_tool_handler` (`RemoteAdapterFFI` / `ConnectResponderFFI`, D16) extend this cdylib FFI surface as a whole.
...
**Tool-handler registry:** `registerToolHandler(capabilityId, handler)` — grammar-validated via `parseToolCapabilityId`: a non-`tools.` id is never registered, and the failure surfaces per face — the TS faces throw on both sides, the **dialer-side** Rust face (`RemoteAdapter::register_tool_handler`) keeps its programmer-error panic, and the **responder-side** Rust face (`ConnectResponder::register_tool_handler`) returns the existing library result `SpokeResult<()>`: the grammar reject passes through verbatim (`INVALID_INPUT`, `details.capability_id`) and `Ok(())` is answered only after the insertion (`SpokeResult` is not `#[must_use]`, so the prevalidating FFI wrapper's discarding call site is unaffected); duplicate registration for the same `capability_id` overwrites (last-wins, documented); the registry MUST NOT mutate the local manifest — the manifest's `tools[]` (carried through hello) is the discovery source. A registry/manifest mismatch is surfaced at invoke time: a manifest-declared tool with no registered handler is denied fail-closed (`op_unsupported` → `CAPABILITY_PORT_MISSING`); `validateManifestTools` checks manifest-internal consistency only. A throwing/rejecting handler answers the error branch — a crash maps to a synthesized `INTERNAL_ERROR` (Rust: `catch_unwind` containment at the future-poll boundary, mirroring the existing invoke path; TS: `toErrorEnvelope`) — an application `SpokeReject` passes through verbatim.
...
**Serving:** the serve loop uses the same gate serialization as the adapter (peek → verify → advance, awaited inline; dispatch fires without blocking the loop). `port.*` ops are served against the injected ports face via the D4 catalogue with the product `op_capability_requirements` map (every baseline `port.*` op requires `spoke-baseline`; `port.computable.project` / `port.computable.compute` require `l2-computable`; `port.fork.list_timeline_events` requires `l5-fork`); the serving order per op is **capability gate → provider probe → serve or deny** — the gate evaluates the required capability against `negotiated_capabilities` (the both-hellos intersection, computed at establish), the probe checks the injected face for the family (structural probe on the injected `BaselinePorts` in TS; `as_computable()` / `as_fork_timeline()` on the `RemoteServePorts` trait in Rust), a family declared in the manifest but not provided by the injected face answers the existing `op_unsupported` dispatch-deny branch (host misconfiguration), and an undeclared family is denied at the gate; absent `ports` still answers `port.*` with the dispatch-deny branch (`op_unsupported`); unknown `port.*` methods / unknown ops → `op_unsupported`. `tools.*` is served through registered handlers per the D13 pipeline. The port success payload is the raw D4 value `T` (the dialer's `invoke_mapped` validates it); the tool success payload is the `{ result }` shape. Deny messages name the offending `port.*` op and, for the optional-family probe, the missing port method, so host misconfiguration is debuggable from the wire. Every served `port.*` provider call then runs inside the responder's **local serve-wait budget** (`invokeTimeoutMs` / `invoke_timeout_ms`, default 5000 ms — the same value as the reverse-invoke waiter deadline): the wait starts after the capability gate, the optional-face probe, the Scope validation and the ownership conjunct, immediately before the provider is dispatched, and covers the whole call. Payload-decode precedence is per face, not shared: the Rust face decodes the op through the D4 catalogue (`decode_port_call`) **before** the bound, so a malformed `port.*` payload keeps its existing `INVALID_INPUT` reject at every budget, zero included; the TS face dispatches through a cast-based `port.*` catalogue with no D4 payload decode at all, so it holds no payload-validation outcome to preserve — at a positive budget the payload reaches the injected provider as-is (the provider owns its interpretation), and at a zero budget the bound is answered before the provider thunk runs, so a malformed baseline payload is answered with the signed serve timeout rather than `INVALID_INPUT`. Expiry answers exactly one signed error response through the existing D7 map (`INTERNAL_ERROR` + `details.kind = "timeout"`) and touches nothing else — the session stays `Established`, the consumed inbound sequence is preserved, and no retry, counter rollback, extra response, capability mutation or forced close follows. A zero budget answers that timeout with zero provider calls. The bound is a bounded wait, not forced termination: Rust drops the timed-out provider future, TS stops awaiting it and consumes the late settlement (timer cleared on every settle path), so a pending provider completion cannot send a late success or a second error. Close is the third cleanup path: the responder releases the session, clears the armed serve timers (TS) and aborts the in-flight serve dispatches (Rust), so no serve wait or its timer outlives the session and a provider completing afterwards answers nothing. Neither language preempts synchronous provider work, a blocked event loop, foreign callbacks, `spawn_blocking` tasks, or rolls back host side effects. Validation, provider absence and capability denial all run first and keep their current codes and provider call counts. Unchanged by this budget: `tools.*` handler waits (responder- and dialer-side), transport send/recv, the handshake, and admission/concurrency quotas.

**`extract` service seam:** the core `extract` op is served by an optional injected service, never by a port-method row. `ConnectResponderOptions.ports` is typed `BaselinePorts & Partial<RemoteExtractService>`, where `RemoteExtractService` is `{ extract(request: ExtractRequest): Promise<SpokeResult<ExtractResponse>> }` (exported from `./remote`); the Rust seam is the connect-owned `RemoteExtractService` trait (`Send + Sync`) probed through `RemoteServePorts::as_extract()` and attached with the consuming `RemoteServePortsComposite::with_extract(Arc<dyn RemoteExtractService>)` — the three-argument `new(baseline, computable, fork)` composes genuine absence, and the blanket impl answers `None`, so a full baseline + optional-family provider never discovers extraction implicitly. The trait is deliberately **not** `spoke_operations::ExtractionPort`: the port is an in-process loader boundary, while this is the responder's service seam, and neither language claims the adapter implements the loader port.

Serving order for `extract` is **capability gate → service probe → decode/validate → call once → signed response**. The gate is the core-table row (`extract → ke-extraction`) evaluated against `negotiated_capabilities`; the probe requires `ports !== undefined && typeof ports.extract === "function"` (Rust `as_extract().is_some()`), and an absent `ports` face and a face without the service are the same dispatch-deny branch (`op_unsupported`) answered **before any source loading** — so a host that declares `ke-extraction` without providing the service denies rather than succeeding emptily. The payload is decoded and validated as `ExtractRequest` before the call; the service is invoked exactly once with `this` bound to the ports object; its `SpokeResult` reject travels the existing application-reject mapping (`toErrorEnvelope` / `to_error_envelope`), and an injected `ExtractResponse` error branch is normalized through the same map to that path (`fromErrorEnvelope` / `from_error_envelope`) — the responder invents no wire code of its own. The success payload is the raw `ExtractResponse` success branch. The whole service call — loader and extractor alike — shares **one** local serve-wait budget (`invokeTimeoutMs` / `invoke_timeout_ms`, the D14 `port.*` budget above): the wait starts at provider invocation and is never restarted per stage, expiry answers the same signed `INTERNAL_ERROR` / `details.kind = "timeout"` response, and a zero budget answers it without starting the service. The Scope validation and the `ke-ownership` conjunct below never apply to `extract`, which carries no Scope. A host's service face may instead run the operations `orchestrate_extract` orchestration itself; because that orchestration future is `!Send` while this face is `Send + Sync`, the documented recipe owns a `Send + Sync + 'static` host clone plus a clone of the active multi-thread runtime handle and drives the orchestration inside a blocking worker (`spawn_blocking` + `Handle::block_on`, the `!Send` future built in that closure) — the worker's `JoinError` maps onto the D7 `INTERNAL_ERROR` row, and the bounded serve wait above stays a wait: it never terminates an already-running worker. Recipe, ownership, `JoinError` mapping and bounded-wait note, with their executable witness (`extract_send_bridge_*`, `crates/spoke-connect/tests/remote_loopback.rs`): [`examples/connect-demo/README.md`](../../examples/connect-demo/README.md#rust-connect-host-bridge-recipe).

**Scope validation + `ke-ownership` conjunct:** a Scope-bearing `port.*` invoke validates its declared Scope first — a present `viewpoint` must be a non-empty string, otherwise the responder answers `INVALID_INPUT` before the provider probe or any host call — and then evaluates the conditional `ke-ownership` requirement against `negotiated_capabilities`, answering `op_unsupported` when the flag is absent. Both checks run after the static capability gate and before the provider is probed or called, and no static core/product gate path returns success ahead of the supplementary check. `port.*` serving otherwise keeps the order and the `op_unsupported` deny shape described above; the ownership requirement is a supplementary conjunct on the existing Scope rows, never a replacement for their row capability.
...
**Public surface:** `invokeTool`, `registerToolHandler`, session info (`sessionId`, `remotePeerId`, `remoteManifest`, `state`), `close`. `registerToolHandler` answers the registration outcome natively — Rust `SpokeResult<()>` (grammar reject passed through verbatim, `Ok(())` only after the insertion), TS `void` with a throw (D13 registry). No envelope-auth/sequence internals exported (`#`-private in TS; no pub internals in Rust).
...
| Dispatch deny — capability not negotiated, or serving side has no registered handler (peer answers `op_unsupported`; `capability_missing` is the same class) | `Rejected { code: "CAPABILITY_PORT_MISSING", kind: None, wire_code: Some("op_unsupported" | "capability_missing") }` — the preserved peer answer code | D13 deny matrix + D7 dispatch-deny row |
...
**Bindings note (the `auth_failed` / `invalid_sequence` rows):** the split follows the shipped D7 mapping — a peer `auth_failed` answer crosses through the envelope-auth row (`INTERNAL_ERROR` + envelope-auth `kind`; `wire_code` stays `None`), while `invalid_sequence` crosses through the unknown-wire-code row (`INVALID_INPUT` + `wire_code`; `kind` stays `None`) — neither row populates both fields.

### D16 — Responder / tool-serving FFI face (ToolHandler + ConnectResponderFFI)

The tool-serving library faces — `ToolHandler` registration and serving on `RemoteAdapter` / `ConnectResponder`, and the `connect_responder` factory (D13/D14) — cross to native binding hosts as the `ToolHandler` callback interface, `RemoteAdapterFFI.register_tool_handler`, and the `ConnectResponderFFI` object; the whole-operation `extract` face (D4) crosses as the dialer-side `RemoteAdapterFFI.extract` and the serving-side `PortsHandler.extract`, joining the D12/D15 surface on the `spoke-connect` cdylib (`ffi` + `remote-adapter` features; generated into the same five native binding channels — C#, Go, Kotlin, Python, Swift). Every method is a sync block-on-async call over the cdylib-owned runtime (D12). `RemoteAdapterFFI.register_tool_handler` is the dialer-side serving face — the registration surface for responder→dialer reverse invokes, the serving direction the bidirectional loopback smokes exercise. Tool arguments and results cross as JSON strings, keys as raw byte arrays, peer ids as strings (D12 boundary conventions).

**Names:** the foreign callback interfaces are **`ToolHandler`** and **`PortsHandler`**, unprefixed — the callback-`Transport` naming precedent: a callback face carries the library-face concept name, while wrapper objects carry the `FFI` suffix (`RemoteAdapterFFI`, `ConnectResponderFFI`). The Rust-side name collision with the library handler types resolves by import alias (`use crate::remote::ToolHandler as RemoteToolHandler`; the ports seam's `RemoteServePorts` needs no alias — the callback trait is `PortsHandler`), the mirror of the existing `use super::foreign_transport::Transport as FfiTransport`. Boundary-type discipline follows the D12 compile face: callback parameters are `Box<dyn Trait>` (the `connect_remote_adapter_ffi(transport: Box<dyn FfiTransport>, …)` precedent; `Arc<dyn …>` parameters are not a compile precedent here), with Rust converting to a long-held `Arc` via `Arc::from(box)` exactly as dial does. Data crosses the boundary only as `String` / `Vec<u8>` / `Vec<String>` / `HashMap<String, Vec<u8>>` / `Option<u64>`, plus object handles and the two callback interfaces.
...
// D16 — responder ports serving face: the exact D4 serve catalogue
// (nine baseline serve ops + the three optional families; getHostCapabilityManifest
// is a session-cache face, not a D4 serve op, and stays out) plus the core-op
// `extract` service face, every method Result<String, FfiError> over JSON-string
// boundaries.
#[uniffi::export(callback_interface)]
trait PortsHandler: Send + Sync {
    fn get_knowledge_entry(&self, entry_id: String) -> Result<String, FfiError>;
    fn put_knowledge_entry(&self, entry_json: String, expected_base_revision: Option<u64>) -> Result<String, FfiError>;
    fn get_relation(&self, relation_id: String) -> Result<String, FfiError>;
    fn put_relation(&self, relation_json: String, expected_base_revision: Option<u64>) -> Result<String, FfiError>;
    ...
    /// Core `extract` service (D4 whole-operation row, F3) — a service face,
    /// not a D4 port method: the callback runs the whole host-local
    /// extraction (loader, extractor, provisional-candidate assembly) and
    /// answers the wire `ExtractResponse` JSON; the loaded input value is
    /// host-local and never a parameter here.
    fn extract(&self, extract_request_json: String) -> Result<String, FfiError>;
}

impl RemoteAdapterFFI {
    /// Dialer-side `extract` (D4 whole-operation row, F1/F3): delegate the
    /// whole extraction to a peer that negotiated `ke-extraction` and return
    /// its `ExtractResponse` success branch as a JSON string. `extract_request_json`
    /// is the `ExtractRequest` itself — no wrapper, and no loader value is an
    /// argument here or a field on the wire. Malformed request JSON rejects
    /// `INVALID_INPUT` at the boundary with zero wire traffic; no local
    /// capability pre-gate — the responder's deny crosses through the D7 rows.
    fn extract(&self, extract_request_json: String) -> Result<String, FfiError>;
...
fn connect_responder_ffi(
    ...
    ports: Option<Box<dyn PortsHandler>>, // OPTIONAL foreign-callback ports face; None → the documented absent-ports deny branch
    invoke_timeout_ms: Option<u64>,       // None → the library default (DEFAULT_INVOKE_TIMEOUT_MS = 5000 ms); one local value: reverse-invoke waiter deadline AND serve-wait budget for served port.* / extract provider calls
...
| `ports: Option<Arc<dyn RemoteServePorts>>` | `ports: Option<Box<dyn PortsHandler>>` | **optional** — mirrors the library `Option`; passing nothing preserves the documented absent-ports deny branch. A supplied callback is bridged via `into_remote_serve_ports` (the `ToolHandler`-analog `spawn_blocking` bridge) and serves the full D4 catalogue: baseline + optional families |
| `invoke_timeout_ms: Option<u64>` | `invoke_timeout_ms: Option<u64>` | passed through (None → library default 5000 ms). One value per responder with two uses: the reverse-invoke waiter deadline **and** the local serve-wait budget bounding served `port.*` provider calls and the whole `extract` service call (D14 Serving — expiry answers the signed `INTERNAL_ERROR` / `details.kind = "timeout"`; zero means immediate serve timeout with zero provider calls) |

**Constructor semantics (library-faithful — not a block-on handshake):** the library `connect_responder` factory returns a `Handshaking` responder immediately (the handshake runs on the background serve loop; the dialer hello is the sync point; the factory has **no error path** — the intentional TS↔Rust divergence recorded in D14). The FFI constructor carries the same semantics: the block-on covers only the factory future, which completes immediately, and the returned responder is in `Handshaking`. The `Result` slot carries FFI-side config-validation failures only — manifest JSON, seed length, or peer-key length → `Dial { kind: "config" }`. Handshake failures (allowlist deny, hello-verify deny) produce **no error row**: they surface as `state() → "Closed"` with the transport closing. A peer that never appears is not a handshake failure — this layer imposes no timeout on the hello `recv`, so the responder stays `Handshaking` until the transport closes (hosts that need a bound apply one at their transport accept layer). Hosts / tests / smokes poll `state()` to `Established` under a bounded wait, matching the TS and Rust consumers.

**Grammar pre-validation on registration (resolved):** over FFI, `capability_id` is foreign input, so `register_tool_handler` returns `Result<(), FfiError>` — a non-`tools.` grammar id rejects `Rejected { code: "INVALID_INPUT", kind: None, wire_code: None }` with the offending id in `message` and zero wire traffic. The wrapper pre-validates with `parse_tool_capability_id` (the `spoke-operations` public API) before the library call, so no library grammar-handling face is reachable from foreign input — the dialer-side panic and the responder-side `SpokeResult<()>` (D13 registry) both stay behind the wrapper, and the FFI signature stays `Result<(), FfiError>` on both wrapper objects. Rationale: the `ffi.rs` convention validates foreign input before calling the library (`parse_json_field` / seed-length precedents); the mapping is symmetric with `invoke_tool` on the same grammar failure (same input, same row); a panic is not a contractable cross-uniffi error channel; and bindings smokes assert an error row instead of catching a crash. The library faces keep their own native semantics — TS throws, and Rust splits by side (dialer panic / responder `SpokeResult<()>`).
...
| Foreign `ToolHandler` returns `Err(Rejected{..})` | Application reject passes through verbatim — `code` / `message` preserved; `kind` / `wire_code` preserved on the `FfiError::Rejected` top-level fields (the inverse of `map_spoke_reject`). A `code` string outside the locked wire vocabulary is not representable in the typed wire enum, so the bridge downgrades it to `INTERNAL_ERROR` with the `message` preserved; every locked code passes through verbatim |
...
| Responder handshake failure (allowlist deny / hello-verify deny) | **No error row** — `state() → "Closed"`, `session_id() → None` (library semantics passthrough; the D14 divergence) |
| FFI responder constructed without a `PortsHandler` receives a `port.*` invoke | Peer answers the documented deny branch → the caller gets `Rejected { code: "CAPABILITY_PORT_MISSING", kind: None, wire_code: Some("op_unsupported") }` |
| Foreign `PortsHandler` returns `Err(Rejected{..})` | Application reject passes through verbatim — `code` / `message` preserved; `kind` / `wire_code` preserved on the `FfiError::Rejected` top-level fields (the inverse of `map_spoke_reject`). A `code` string outside the locked wire vocabulary is not representable in the typed wire enum, so the bridge downgrades it to `INTERNAL_ERROR` with the `message` preserved; every locked code passes through verbatim. A callback rejecting an op it does not serve is ordinary deny, not containment |
| Foreign `PortsHandler` returns `Err(Dial{..})` / a non-contract exception / panics / the bridge sees `JoinError` | `Rejected { code: "INTERNAL_ERROR", kind: None }` — containment, `details: None` mirroring the library `catch_unwind`; the serve loop survives |
| Foreign `PortsHandler` returns malformed success JSON for a served op | `Rejected { code: "INTERNAL_ERROR", kind: None }` — containment (foreign-produced output never lands on an async worker; it parses inside the same blocking task) |

**Responder ports serving face (`PortsHandler`, shipped):** `connect_responder_ffi` takes an OPTIONAL `ports: Option<Box<dyn PortsHandler>>` — the D4 serve catalogue as a synchronous foreign callback (thirteen methods: the twelve D4 serve methods — the nine baseline serve ops plus `project` / `compute` / `list_fork_timeline_events` — plus the core-op `extract` service; `getHostCapabilityManifest` is not a D4 serve op and stays out). `into_remote_serve_ports` bridges the callback into the library `RemoteServePorts` seam with the `ToolHandler` threading + outcome model: `Arc::from(box)` long-hold, every call on the runtime's `spawn_blocking` pool, `Ok(json)` parsed to the typed success value inside the same blocking task, `Err(Rejected{..})` passing through verbatim, `Err(Dial{..})` / `JoinError` contained to `INTERNAL_ERROR`. The returned face is an explicit service composite — `RemoteServePortsComposite::new(baseline, Some(computable), Some(fork)).with_extract(extract)`: one callback object behind four faces (unsize coercions of the same `Arc`), and composing through the composite is what opts extraction in, since the operations-crate full-optional blanket impl answers `None` from `as_extract()` and a bare bridge would read as an absent provider. The bridge always presents the computable / fork faces (the probes return the bridge) — capability gating stays in the library responder (gate → probe → serve/deny), and a callback rejecting an op it does not serve is ordinary deny, not containment. The optional dialer faces (`RemoteAdapterFFI.project` / `compute` / `list_fork_timeline_events`) mirror the library delegation 1:1 with `parse_json_field` pre-validation and no local pre-gate, and `RemoteAdapterFFI.extract` mirrors the D4 whole-operation delegation under the same boundary rule; on the router FFI the optional families ride the per-peer adapter — a consumer lists peers via `list_peers` and drives the optional ops on the corresponding `RemoteAdapterFFI` (the router's capability gate tables keep the ops capability-mapped and never ungated). Re-entrancy caveat: a binding must NOT call back into the FFI surface from inside a ports callback (the `ToolHandler` rule, D16 §8.5) — ports callbacks behave like tool handlers, one blocking-pool thread per in-flight callback.

**Three distinct `extract` unavailability outcomes (shipped):** an `extract` invoke that cannot be served settles on one of three rows, each distinguishable at the binding:

| Outcome | Condition | `FfiError` row | Foreign callback calls |
|---|---|---|---|
| Capability-gate deny | `ke-extraction` absent from the negotiated intersection (either hello omits it) | `Rejected { code: "CAPABILITY_PORT_MISSING", kind: None, wire_code: Some("op_unsupported") }` | 0 — the static core-table gate precedes any host work, even when a callback exists |
| Absent-provider probe deny | negotiated, the optional `ports` parameter omitted (`None`) | the same `CAPABILITY_PORT_MISSING` + `wire_code: Some("op_unsupported")` row | none — no callback exists |
| Callback application refusal | negotiated, the callback answers `Err(Rejected{..})` (the binding does not serve extraction) | `CAPABILITY_PORT_MISSING`, `wire_code: None`, the callback `message` preserved | exactly 1 |

A callback declining an op it does not serve is ordinary application deny — never recast as a missing-method probe or as containment.

**Ownership conjunct over FFI (`list_knowledge_entries` witness, shipped):** the conditional `ke-ownership` requirement a viewpoint-bearing Scope adds needs no FFI surface of its own — it rides the existing Scope callback methods, with `list_knowledge_entries(scope_json)` as the shipped witness. With `ke-ownership` negotiated the declared Scope JSON reaches the callback unchanged (a `viewpoint`-bearing request is never stripped to make it succeed) and the callback is reached exactly once; when either hello omits the flag, the invoke settles on the gate-deny row above with zero callback calls.
...
- True-async FFI and listeners inside the FFI surface are out of scope.

### D17 — Validator helpers stay library-level (no FFI export)

The FFI surface (D12/D15/D16) carries the session faces — `RemoteAdapterFFI` / `MultiPeerRouterFFI` / `ConnectResponderFFI`, the callback `Transport` and `ToolHandler` interfaces, `invoke_tool`, and `register_tool_handler`. The pure validators `validateManifestTools` and `validateToolArguments` are library-level host-side helpers in `@42ch/spoke-operations` / `spoke-operations` (public API in both languages, with dedicated test coverage) and are not exported over FFI. They sit outside the parity surface as thin conveniences (`spoke-connect-ts-route.md` §Helper boundary (intentional asymmetry), L133: thin client conveniences are outside session-core parity), and the connect library adds no pure helpers of its own (`spoke-connect.md` §Relationship to the existing three pillars: connect does not add pure helpers; remote dispatch stays adapter-owned). What the wire path does guarantee is authentication, negotiation, and fail-closed gating — not structural validation: the signed hello cryptographically authenticates the embedded manifest at session establish (signature + identity verify), the negotiated capability set is the intersection of the two authenticated hello manifests, and the `tools.*` dispatch gate evaluates each op against the negotiated capabilities, denying fail-closed — a manifest-declared-but-unregistered tool answers `op_unsupported` → `CAPABILITY_PORT_MISSING` (D13 deny matrix) at invoke time, and the handler-registration gate admits only `tools.` ids. Structural manifest/tool-descriptor validation — namespace ownership, `tools[].capability_id ∈ capabilities[]` membership, descriptor uniqueness, `op === capability_id` — is performed only by the operations-library helpers (`validateManifestTools` / `validateToolArguments`); native hosts that want pre-dial or pre-invoke validation call those helpers directly in their own language, and the wire-path gates are not a substitute for those calls.
...
- **Responder (D14):** `connectResponder` / `connect_responder` — same options shape, allowlist-first handshake recipe, `port.*` serving via the D4 catalogue (each served provider call and the whole `extract` service call run inside the responder's local serve budget — expiry answers one signed `INTERNAL_ERROR` / `details.kind = "timeout"` and leaves the session usable in both languages; Rust drops the timed-out provider future, TS stops awaiting and consumes the late settlement), reverse tool face, session info + `close`; **intentional divergence** — TS returns `Promise<ConnectResponder>` (instance), Rust returns `Arc<ConnectResponder>` with **no `Result`** (the TS seed-length throw is type-enforced via `[u8; 32]`, so the error slot is uninhabitable); handshake failures surface via `state → Closed` + transport close in both; registration grammar failure is native to each language (TS throws / Rust returns the library `SpokeResult<()>`).
...
- **FFI mirror (D15/D16):** the FFI surface mirrors the locked D13–D16 library faces — `invoke_tool` on the adapter / router / responder, `register_tool_handler`, and the `connect_responder_ffi` factory — as synchronous block-on-async calls over the cdylib-owned runtime (D12); native bindings consume only the FFI surface, and the mirror adds no behavior of its own (error rows extend the D7 mapping by reference), so the TS↔Rust parity rows remain the D13/D14 library rows above.
...
1. Consumer-side WebSocket Transport packages
...
- [x] D15: `invoke_tool` invoked via FFI — `RemoteAdapterFFI.invoke_tool` / `MultiPeerRouterFFI.invoke_tool` generated in all five bindings as synchronous block-on-async calls over the cdylib-owned runtime (JSON-string `arguments_json` in, tool `result` payload out), grammar fail-fast `INVALID_INPUT` with zero wire traffic, router terminal `no_capable_peer` reject, D7-by-reference error rows (five-binding generated faces carry `invoke_tool`; per-channel DEFAULT-suite tool smokes drive it — each channel's smoke README + harness; Rust FFI test battery incl. router tool tests `ffi.rs` L5359 / L5423 / L5490 / L5581)
- [x] D16: `tools.*` served via FFI — `ToolHandler` callback interface, `RemoteAdapterFFI.register_tool_handler` (dialer-side serving), and `ConnectResponderFFI` (`connect_responder_ffi` factory, session info, `register_tool_handler` / `invoke_tool` / `close`) generated in all five bindings; accept topology = host-owned listen/accept wrapping a connected callback `Transport` (loopback proofs over `loopback_transport_pair()`) (five-binding generated faces carry `ToolHandler` / `register_tool_handler` / `ConnectResponderFFI`; per-channel DEFAULT-suite tool smokes serve both directions over `loopback_transport_pair()` — each channel's smoke README + harness; Rust FFI test battery `ffi.rs` L3965–L4156 dialer-side serving + L4383–L4830 responder pair)
- [x] Error rows: the D15/D16 `FfiError` rows hold at the bindings — grammar rejects, dispatch deny (`wire_code: "op_unsupported"`), envelope-auth kinds, `invalid_sequence` wire_code, handler `Rejected` pass-through with `Dial` / exception / panic containment, config-validation `Dial { kind: "config" }`, responder handshake failure via `state → "Closed"` (no error row) (Rust FFI boundary battery — `ffi.rs` L2720–L3020 error parity, L3458–L3755 tool-face rows, L4383–L4830 responder rows, L5359–L5629 router rows; per-channel DEFAULT-suite smokes assert the binding-specific fielded rows: deny `wire_code` + handler `Rejected` passthrough with code/message/kind/wire_code, and unknown reject code → `INTERNAL_ERROR` downgrade (message preserved, details re-hung))
- [x] Containment: panics inside block-on-async futures and non-contract handler failures never unwind across the FFI boundary (`INTERNAL_ERROR` containment rows; the serve loop survives); concurrent calls from multiple foreign threads multiplex over the shared runtime (Rust FFI containment battery — `ffi.rs` L3602–L3640 concurrent OS-thread invokes, L4055–L4098 handler `Dial` / panic containment with serve-loop survival proven by a post-containment healthy invoke; library `catch_unwind` containment `remote_loopback.rs` L2434–L2470; per-channel handler `Rejected` passthrough + foreign-fault containment with serve-loop survival (post-containment healthy reverse invoke))
- [x] FFI parity mirror statement recorded in the TS↔Rust parity section above (the FFI surface mirrors the locked D13–D16 library faces; native bindings consume only the FFI surface; sync block-on-async per D12)
- [x] Optional-port dialer delegation (library faces): `RemoteAdapter` drives `project` / `compute` / `listForkTimelineEvents` over the existing session invoke path — `port.computable.project` / `port.computable.compute` / `port.fork.list_timeline_events` with snake_case payloads, no local dialer pre-gate, D7 deny mapping; loopback round-trip + capability-gate deny per op in both languages (TS `tests/remote/remote-adapter.test.ts` "round-trips project over the loopback (port.computable.project, snake_case payload)" and the compute / fork round-trips + the three "maps the capability-gate deny for …" negatives; Rust `tests/remote_loopback.rs` `round_trips_project_over_the_loopback` / `round_trips_compute_over_the_loopback` / `round_trips_list_fork_timeline_events_over_the_loopback` + the three `maps_the_capability_gate_deny_for_*` negatives)
- [x] Router fork gate row (library faces): `port.fork.list_timeline_events` → `l5-fork` hard-filter row in both routers, inert for the fixed six-family surface; capability-mapped `no_capable_peer` when no peer advertises `l5-fork`, unknown-op regression intact (TS `tests/remote/multi-peer-router.test.ts` "selects the peer whose manifest advertises l5-fork (gate-table row, inert for the six-family surface)" / "rejects with the locked no_capable_peer when no peer advertises l5-fork — capability-mapped, not unknown-op" / "keeps the no_capable_peer regression for ops outside the table (never falls through ungated)"; Rust `src/remote/multi_peer_router.rs` `selects_the_peer_whose_manifest_advertises_l5_fork` / `rejects_with_no_capable_peer_when_no_peer_advertises_l5_fork` / `keeps_the_no_capable_peer_regression_for_ops_outside_the_table`)
- [x] Responder optional-family serving (library faces): the serve catalogue covers the optional families with the gate → probe → serve/deny order — loopback round-trip per family, undeclared-capability gate deny, declared-but-not-provided probe deny, baseline smoke green in the same session (TS `tests/remote/responder.test.ts` "connectResponder optional-port serving (gate → probe → serve/deny)" suite — "round-trips project through the responder (port.computable.project, snake_case payload) while baseline serving stays green", the compute / fork round-trips, the three "denies … when … is not negotiated (gate deny → op_unsupported → CAPABILITY_PORT_MISSING + wire_code)" negatives, and "denies all three optional ops when the capability is declared but the provider is baseline-only (probe deny → op_unsupported → CAPABILITY_PORT_MISSING + wire_code)"; Rust `tests/remote_loopback.rs` `responder_round_trips_project_while_baseline_serving_stays_green` / `responder_round_trips_compute_over_the_loopback` / `responder_round_trips_list_fork_timeline_events_over_the_loopback` + the three `responder_denies_*` gate negatives + `responder_denies_all_three_optional_ops_when_the_capability_is_declared_but_the_provider_is_baseline_only`)
- [x] FFI optional dialer delegation: `RemoteAdapterFFI.project` / `compute` / `list_fork_timeline_events` generated in all five bindings as synchronous block-on-async calls (JSON-string args in, JSON-string results out, `parse_json_field` pre-validation with zero wire traffic, no local pre-gate, D7 deny mapping `CAPABILITY_PORT_MISSING` + `wire_code: "op_unsupported"`); loopback round-trip + capability-gate deny per op (Rust FFI battery — `remote_adapter_ffi_round_trips_{project,compute,list_fork_timeline_events}_over_the_loopback` + `remote_adapter_ffi_maps_the_capability_gate_deny_for_optional_ops` + `remote_adapter_ffi_optional_ops_reject_malformed_json_with_zero_wire_traffic`; per-channel DEFAULT-suite smokes drive the round-trips + denies — each channel's smoke README + harness; verified 2026-08-18 — five-channel DEFAULT suites green, xcframework refreshed via CI run 32105017899 (apply) + drift-green re-run 32105616809). Optional families ride the per-peer adapter on the router FFI (`list_peers` → the corresponding `RemoteAdapterFFI`) — the router surface stays the locked fixed six-family face with capability-mapped gate rows
- [x] FFI responder ports serving: `PortsHandler` callback interface (twelve D4 serve ops, `Result<String, FfiError>`) + the optional `ports` parameter on `connect_responder_ffi` generated in all five bindings; loopback round-trips through the foreign callback for baseline + optional families, absent-ports deny branch preserved when the parameter is omitted, application `Rejected` passthrough, foreign-fault containment with serve-loop survival (Rust FFI battery — `connect_responder_ffi_serves_{baseline,optional_port_families}_through_a_foreign_ports_handler` + `connect_responder_ffi_contains_a_panicking_foreign_ports_handler_and_the_session_survives` + the absent-ports deny test; per-channel DEFAULT-suite smokes serve baseline + optional through the callback — each channel's smoke README + harness; verified 2026-08-18 — five-channel DEFAULT suites green, xcframework refreshed via CI run 32105017899 (apply) + drift-green re-run 32105616809)
- [x] Serve-side timeout boundary (library faces): every served `port.*` provider call and the whole `extract` service call are bounded by the responder's local serve budget (`invokeTimeoutMs` / `invoke_timeout_ms`, default 5000; one value shared with the reverse-invoke waiter deadline) — expiry answers exactly one signed `INTERNAL_ERROR` / `details.kind = "timeout"` response, keeps the session `Established` and the consumed inbound sequence, and discards the late completion; a zero budget answers before any provider call while the capability gate, the optional-face probe, the Scope validation, the ownership conjunct and — on the Rust face only — the D4 payload decode keep their precedence (Rust `decode_port_call` runs before the bound, so a malformed `port.*` payload stays `INVALID_INPUT` at any budget; the TS face dispatches through a cast-based `port.*` catalogue with no D4 payload decode at all, so it holds no payload-validation outcome to preserve and its zero-budget early return precedes the provider thunk — a malformed baseline payload is answered by the signed serve timeout with zero provider calls), and a close clears the armed serve timers (TS) / aborts the in-flight serve dispatches (Rust) so a provider settling afterwards answers nothing (TS `tests/remote/responder.test.ts` "serve timeout (local serve budget)" suite — the parked Scope-port / optional-family / extract cases, "zero budget answers the signed timeout before any provider call", "zero budget keeps gate, probe and ownership denials ahead of the bound", "zero budget answers the signed serve timeout for a malformed baseline port payload", "does not bound a provider call that completes inside the budget", "answers nothing when the provider settles after the responder closed", "disarms the serve budget when the responder closes mid-wait"; Rust `tests/remote_loopback.rs` `serve_timeout_bounds_a_parked_scope_port_and_answers_one_signed_timeout` / `serve_timeout_bounds_a_parked_optional_family_provider` / `serve_timeout_bounds_a_parked_extract_service_and_discards_the_late_completion` / `serve_timeout_zero_budget_answers_before_any_provider_call` / `serve_timeout_zero_budget_keeps_gate_and_deny_precedence` / `serve_timeout_zero_budget_keeps_scope_validation_precedence` / `serve_timeout_zero_budget_keeps_port_payload_validation_precedence` / `serve_timeout_close_aborts_a_parked_serve_dispatch_and_answers_nothing` / `serve_timeout_does_not_affect_a_normal_serve_call`)
