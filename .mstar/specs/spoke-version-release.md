...
| 12 | Cargo lockfile | `Cargo.lock` → `[[package]]` for `spoke-schemas`, `spoke-operations`, `spoke-fixture-toy-world`, `spoke-connect` | TOML package version entries |
| 13 | README EN badge | `README.md` | Dynamic shields.io GitHub Releases badge (presence) |
| 14 | README CN badge | `README_CN.md` | Same as row 13 |
...
CI **lockstep assert** MUST cover rows 1–14. Drift on any row MUST fail the build (no warn-only path). `release:bump` MUST rewrite `Cargo.lock` member versions when bumping (Node-only; no `cargo` required).
...
Release workflows publish `@42ch/spoke-schemas`, `@42ch/spoke-operations`, `@42ch/spoke-connect`, `spoke-schemas`, `spoke-operations`, `spoke-connect`, and GitHub Packages NuGet `42ch.Spoke.Connect`. Fixture and codegen packages remain private. Third-party Actions MUST pin by commit SHA (same policy as `ci.yml`).
...
| Job layout | Optional `tag` → `resolve-tag` → **four parallel verify jobs** → **`release`** → **`publish-npm`** then **`publish-crates`**, plus **`build-connect-ffi`** matrix → **`publish-nuget`** (tags without `-rc.` only) |
