...
- Stable tags (`vX.Y.Z`) and prerelease SemVer tags without `-rc.` (e.g. `v0.1.0-alpha.3`) publish to npm and crates.io; tags containing `-rc.` create GitHub pre-releases only. If `publish-crates` fails after npm succeeded, re-run **Release** via `workflow_dispatch` with that tag.
...
- **Release** (`release.yml`) is the sole top-level publish workflow: `push.tags: v*`, `pull_request` closed + label `release`, or `workflow_dispatch` with `tag` (recovery). Do not wrap it in `workflow_call` — npm/crates Trusted Publishing OIDC binds to the top-level workflow filename (`release.yml`, same pattern as mstar-harness). npm publish: Node 24 + `registry-url`, stock npm (do not `npm install -g npm@…`).
