...
- Connect (`schemas/connect/` + `spoke-connect` flag) is **wire + normative spec only**: no daemon, no runtime, no published connect package; a reference spike will live in `crates/spoke-connect` when it lands (unpublished, `publish = false`); product connect bindings ship in consumer repositories.
