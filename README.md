...
3. Merge the opened PR (keep the `release` label), or close it to abort. CI creates annotated tag `vX.Y.Z` and runs [**Release**](https://github.com/42ch-dev/spoke/actions/workflows/release.yml): verify gates, GitHub Release from [`CHANGELOG.md`](CHANGELOG.md), and on stable / alpha tags (no `-rc.` suffix) publish npm + crates.io.
