...
**Standardized Programmable Ontology Keyblock Engine** — 这是一个**协议仓库**，不是运行时。SPOKE 用 JSON Schema 定义叙事 **Keyblock** 数据层与 **ops** 操作层的线型契约，使 Nexus、Creader 等产品能在不共享数据库、守护进程或部署的前提下，交换一致性检查与上下文组装的 I/O。
...
从 `@42ch/spoke-schema` 导入线型类型，从 `@42ch/spoke-operations` 导入生命周期辅助函数：
...
| [`.mstar/specs/spoke-ops.md`](.mstar/specs/spoke-ops.md) | Ops 线型请求/响应信封 |
