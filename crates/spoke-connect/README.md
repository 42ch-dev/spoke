...
### Additive responder surface (sync `ConnectResponderFFI`)

The accept side mirrors the dialer: `connect_responder_ffi` wraps a
*connected* (host-accepted) callback `Transport` into the encapsulated
`ConnectResponder` lifecycle — symmetric with `connect_remote_adapter_ffi`,
which takes a connected outbound transport; the host product owns
listen/accept in its own network stack. The constructor resolves immediately
...
The landed binding surface is **core + remote adapter + responder**: the sync
session-core table above plus the additive `RemoteAdapterFFI` /
`MultiPeerRouterFFI` / `ConnectResponderFFI` objects, the callback `Transport`
/ `ToolHandler` interfaces, and the `FfiError` / `TransportError` enums when
`remote-adapter` is enabled. Host languages can either implement their own
transport against the wire contract using the core-only helpers, or supply a
callback `Transport` and dial through `connect_remote_adapter_ffi` for the
full encapsulated adapter — the accept side is symmetric: a host-accepted
transport enters through `connect_responder_ffi`, serving `tools.*` from a
foreign `ToolHandler`. The **core + async node** option — additionally
bridging a thin `SpokeConnectNode` lifecycle over the uniffi
async/foreign-runtime mechanism — is deferred: node start/listen/shutdown and
`connect(addr)` stay Rust-side today.
