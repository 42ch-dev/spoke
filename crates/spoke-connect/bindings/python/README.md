...
| Wheel platform | Native | Status |
|----------------|--------|--------|
| `macosx_arm64` | `spoke_connect/libspoke_connect.dylib` | **Committed** — maintainer-built (release) |
| `linux_x86_64` | `libspoke_connect.so` | **CI** — stage from `release.yml` `build-connect-ffi` (`linux-x64`) before wheel build |
