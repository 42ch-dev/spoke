...
mod tests {
    ...
    fn signed_fields_are_jcs_deterministic() {
        ...
        let hello_a = sign_hello(&a, &nonce, &manifest_a, None).expect("sign a");
        let hello_b = sign_hello(&b, &nonce, &manifest_a, None).expect("sign b");
    ...
    fn responder_hello_dial_binding_round_trip() {
        ...
        let initiator_nonce = test_nonce();
        let responder_nonce = test_nonce();
