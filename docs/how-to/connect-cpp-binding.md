...
The three blocks below compose one complete `host.cpp`: a host-owned queue transport, the demo identity with a served echo tool, and the dial with one real invoke. Append them in order — every include and helper they use is shown. A failure before a serving handle exists ends the process with the underlying status; every failure after `ConnectResponder::serve` has returned is reported, closed in order, and returned as the process's exit status.
