...
| `modules.*` | KnowledgeEntry、AssemblePacket 与 TimelineEvent 上的可选 `ModuleMap`（能力标志 `narrative-modules`）；键为功能方言 id（`activation`、`placement`、`activation_trace`、`mental`、`belief`、`observation` 等）；值为结构化 JSON，内部形状由手册定义 | 叙事主机共享的跨产品功能方言。未知模块键原样往返 |
